...
def dw_conv3x3(in_channels, out_channels, module_name, postfix,
    ...
    out_channels1 = int(round(in_channels * 6))
    return [
...
def dconv3x3(in_channels, out_channels, module_name, postfix,
              stride=1, groups=1, kernel_size=3, padding=1):
    """3x3 convolution with padding"""
    return [
        (f'{module_name}_{postfix}/conv',
...
class _OSA_stage(nn.Sequential):
    def __init__(
        ...
        if not stage_num == 2:
            ...
            #self.add_module("Pooling", BlurPool(in_ch))
...
class VoVNet(Backbone):
    def __init__(self, cfg, input_ch, out_features=None):
        ...
        # Stem module
        conv_type = dconv3x3 if depthwise else conv3x3
        stem = dconv3x3(input_ch, stem_ch[0], "stem", "1", 2)
        stem += conv_type(stem_ch[0], stem_ch[1], "stem", "2", 1)
        stem += conv_type(stem_ch[1], stem_ch[2], "stem", "3", 2)
        self.add_module("stem", nn.Sequential((OrderedDict(stem))))
        current_stirde = 4
        self._out_feature_strides = {"stem": current_stirde, "res2": current_stirde}
        ...
        for i in range(4):  # num_stages
            name = "res%d" % (i + 2)  # stage 2 ... stage 5
            ...
            self.aspp.append(ASPP(config_concat_ch[i], config_concat_ch[i]))
